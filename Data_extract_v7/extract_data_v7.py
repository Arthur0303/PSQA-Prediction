...
COLUMNS = [
    STUDY_ID_COLUMN, "Date", "Plan", "Site", "Energy", "ArcCheck", "ModFactor",
...
def _row_key(row: dict) -> tuple:
    study_id = row.get(STUDY_ID_COLUMN)
    if study_id is None:
        study_id = row.get(LEGACY_ID_COLUMN)
    return (study_id, row.get("Date"), row.get("Site"))
...
def _load_existing_rows(output_path: str) -> list[dict]:
    ...
    has_study_id_header = STUDY_ID_COLUMN in headers
    rows = []
    for values in ws.iter_rows(min_row=2, values_only=True):
        ...
        for idx, header in enumerate(headers):
            if idx >= len(values):
                continue
            if header == LEGACY_ID_COLUMN and has_study_id_header:
                continue
            column = STUDY_ID_COLUMN if header == LEGACY_ID_COLUMN else header
            if column in row_map:
                row_map[column] = values[idx]
...
def save_to_xlsx(records: list[dict], output_path: str) -> list[str]:
    """Accumulate extracted records into xlsx, replacing matching Study ID/Date/Site rows."""
...
def _print_intro() -> None:
    ...
    print("      Study ID + Date + Site are updated in place; new rows are")
    print("      appended.")
    ...
    print("    - For a single PDF with at most one TPS DICOM and at most")
    print("      one Structure Set, the selected DICOM files are used")
    print("      directly; omitted DICOM fields are left blank.")
    print()
...
def main() -> None:
    ...
    single_case_passthrough = (
        len(pdf_files) == 1 and len(dcm_files) <= 1 and len(rtstruct_files) <= 1
    )
    ...
    if not single_case_passthrough and len(dcm_files) != len(pdf_files):
        errors.append(
    ...
    for pdf_path in pdf_files:
        ...
        if single_case_passthrough:
            tps_plan = dcm_files[0] if dcm_files else None
        else:
            tps_plan = find_tps_plan(dcm_files, pid, calc)
        if tps_plan is None:
            if not single_case_passthrough:
        ...
        if single_case_passthrough:
            rtstruct = rtstruct_files[0] if rtstruct_files else None
        else:
